...
# Buang $ dari kolom Price, ubah jadi float
playstore['Price'] = playstore['Price'].apply(lambda x: x.replace("$",""))
playstore['Price'] = playstore['Price'].astype('float')

# Ubah tipe data Reviews & Size ke dalam tipe data integer
...
def index():
    ...
    grouped = df2.groupby(['Content Rating', 'Type']).size().unstack().sort_values("Free")
    fig = plt.figure(figsize=(12,7))
    fig.add_subplot()
    grouped.plot.barh(ax=plt.gca())
    plt.xlabel('App Count')
    plt.ylabel('Content Rating')
    plt.legend(loc='lower right')
    plt.savefig('contrate_type.png', bbox_inches="tight")
